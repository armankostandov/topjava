...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_BETWEEN, query = "SELECT m FROM Meal m " +
                "WHERE m.user.id=:user_id AND m.dateTime<:endDateTime AND " +
                "m.dateTime>=:startDateTime ORDER BY m.dateTime DESC"),
        ...
        @NamedQuery(name = Meal.UPDATE, query = "UPDATE Meal m SET m.dateTime=:dateTime, " +
                "m.description=:description, m.calories=:calories WHERE " +
                "m.user.id=:user_id AND m.id=:id")
    })
@Entity
